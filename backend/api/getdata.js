...
router.get("/getdata", async (req, res) => {
  ...
  for (i in array) {
    ...
    while (count < stop) {
      ...
      if (count < stop - 1) {
        ...
        if (next_s[0][0].type != s_type) {
          ...
          next_s[0][0].station_line != table[0][0].station_line &&
          next_s[0][0].type != "SRT" &&
          s_type != "SRT"
        ) {
          tran = "true";
      ...
      if (s_type == "SRT") {
        let query1 = await pool.query(
          "select used_time, station_zone from srt where station_id = ?",
          [data[count]]
        );
      ...
      if (
        (s_type == "SRT" && count == 0 && name == "กรุงเทพ") ||
        name == "บางซื่อ"
      ) {
        total_cost += 2;
      }
      if (s_type == "SRT") {
        if (table[0][0].station_line == 1) {
          srt_line1 += 1;
        } else if (table[0][0].station_line == 2) {
          srt_line2 += 1;
        } else if (table[0][0].station_line == 3) {
          srt_line3 += 1;
        } else if (
          table[0][0].station_line == 4 ||
          table[0][0].station_line == 5
        ) {
          srt_line4_5 = "true";
        }
      } else if (s_type == "ARL") {
        ...
        if (table[0][0].station_line == 2) {
          ...
          if (
            (table[0][0].station_id <= 153 && table[0][0].station_id >= 145) ||
            (table[0][0].station_id <= 178 && table[0][0].station_id >= 170) ||
            (table[0][0].station_id <= 202 && table[0][0].station_id >= 193)
          ) {
            bts_line1 += 1;
          } else if (
            table[0][0].station_id <= 168 &&
            table[0][0].station_id >= 154
          ) {
            bts_line2 += 1;
          } else if (
            table[0][0].station_id <= 192 &&
            table[0][0].station_id >= 179
          ) {
            bts_line3 += 1;
          } else if (
            table[0][0].station_id <= 206 &&
            table[0][0].station_id >= 203
          ) {
            bts_line4 += 1;
          }
        }
    ...
    if (srt_line1 != 0 || srt_line2 != 0 || srt_line3 != 0) {
      srt_cost =
        (14 * srt_line1) / 13 + (8 * srt_line2) / 14 + (6 * srt_line3) / 14;
    }
    if (srt_line4_5 == "true") {
      srt_cost += 10;
    }
    ...
    if (bts_line1 != 0 || bts_line2 != 0 || bts_line3 != 0 || bts_line4 != 0) {
      if (bts_line1 != 0) {
        if (bts_line1 - 1 >= 8) {
          bts_cost += 44;
        } else if (bts_line1 - 1 >= 6) {
          bts_cost += (bts_line1 - 1) * 3 + 3 + 16;
        } else if (bts_line1 - 1 >= 4) {
          bts_cost += (bts_line1 - 1) * 3 + 2 + 16;
        } else if (bts_line1 - 1 >= 2) {
          bts_cost += (bts_line1 - 1) * 3 + 1 + 16;
        } else {
          bts_cost += 16;
        }
        tran_bts += 1;
      ...
      if (bts_line2 != 0) {
        if ((bts_line2 - 1) * 3 + 15 >= 45) {
          bts_cost += 45;
        } else {
          bts_cost += (bts_line2 - 1) * 3 + 15;
        }
        tran_bts += 1;
      }
      if (bts_line3 != 0) {
        if ((bts_line3 - 1) * 3 + 15 >= 45) {
          bts_cost += 45;
        } else {
          bts_cost += (bts_line3 - 1) * 3 + 15;
        }
        tran_bts += 1;
      }
      if (bts_line4 != 0) {
        if ((bts_line4 - 1) * 3 + 15 >= 24) {
          bts_cost += 24;
        } else {
          bts_cost += (bts_line4 - 1) * 3 + 15;
        }
        tran_bts += 1;
      }
    ...
    if (blue_line != 0 || purple_line != 0) {
      if (blue_line - 1 >= 12) {
        blue_cost += 42;
      } else if (blue_line - 1 >= 10) {
        blue_cost += (blue_line - 1) * 2 + 18;
      } else if (blue_line - 1 >= 7) {
        blue_cost += (blue_line - 1) * 2 + 17;
      } else if (blue_line - 1 >= 4) {
        blue_cost += (blue_line - 2) * 2 + 18;
      } else {
        blue_cost += (blue_line - 2) * 2 + 17;
      }

      if (purple_line - 1 >= 11) {
        purple_cost += 42;
      } else if (purple_line - 1 >= 9) {
        purple_cost += (purple_line - 1) * 2 + 3 + 17;
      } else if (purple_line - 1 >= 6) {
        purple_cost += (purple_line - 1) * 3 - 5 + 17;
      } else if (purple_line - 1 >= 4) {
        purple_cost += (purple_line - 1) * 2 + 17;
      } else {
        purple_cost += (purple_line - 2) * 3 + 17;
      }
    }
    if (blue_line != 0 && purple_line != 0) {
